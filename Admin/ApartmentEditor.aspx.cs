...
namespace Admin
{
	public partial class ApartmentEditor : System.Web.UI.Page
	{
		...
		{
			ddlApartmentOwner.DataSource = _apartmentOwnerRepository.GetApartmentOwnersRaw();
			ddlApartmentOwner.DataBind();
		}

		private void RebindCities()
		{
			ddlCity.DataSource = _cityRepository.GetCitiesRaw();
			var temp = ddlCity.DataSource.ToString();
			ddlCity.DataBind();
		}

		private void RebindStatuses()
		{
			ddlStatus.DataSource = _statusRepository.GetStatusesRaw();
			ddlStatus.DataBind();
		}
		...
		private List<Models.ApartmentPicture> GetRepeaterPictures()
		{
			...
			foreach (RepeaterItem item in repApartmentPicturesItems)
			{
				...
				if (pic.DoDelete)
				{
					_apartmentRepository.DeletePicture(int.Parse((item.FindControl("hidApartmentPictureId") as HiddenField).Value));
				}
				else
				{
					pictures.Add(pic);
